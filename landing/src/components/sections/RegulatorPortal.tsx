  ...
  return (
    <div className="tool flex h-full flex-col">
      <header className="tool-head">
        ...
        <span className="truncate font-mono text-[11.5px] text-steel-300">{c.passportId}</span>
      </header>

      <div className="px-4 pt-4">
        <p className="font-body text-[12.5px] text-steel-300">{c.prompt}</p>
        <div role="tablist" aria-label="Field to disclose" className="mt-2 flex flex-wrap gap-1.5">
          {c.fields.map((x, k) => (
            <button key={x.key} role="tab" type="button" aria-selected={k === i} onClick={() => setI(k)} className="chip font-body text-[12.5px]">
              {x.key}
            </button>
          ))}
        </div>
      ...
      <div className="mx-4 mt-5 rounded-[6px] px-4 py-4" role="tabpanel" style={{ boxShadow: "inset 2px 3px 8px rgba(0,0,0,0.6), inset -1px -1px 1px rgba(255,255,255,0.05)", background: "#171a1f" }}>
        <p className="font-body text-[12px] text-steel-300">{f.key}</p>
        <p className="struck-ink mt-1 font-display text-[30px] font-extrabold leading-none tabular-nums">{f.value}</p>
        <p className="mt-2 font-body text-[12px] text-steel-300">{f.note}</p>
      </div>

      <ul className="mt-4 flex flex-col gap-1.5 px-4 font-body text-[12px] text-steel-300">
        {c.fields.map((x, k) => (
      ...
      <p className="mt-auto px-4 pb-4 pt-5 font-body text-[12.5px] text-steel-300">{c.hint}</p>
    </div>
  );
}
