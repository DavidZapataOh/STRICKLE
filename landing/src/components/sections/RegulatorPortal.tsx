...
export function RegulatorPortal() {
  ...
  return (
    <div className="tool flex h-full flex-col">
        ...
        <div role="tablist" aria-label="Field to disclose" className="mt-2 flex flex-wrap gap-1.5">
          {c.fields.map((x, k) => (
            <button
              ...
              aria-selected={k === i}
              onClick={() => setI(k)}
              className="chip font-body text-micro"
            >
              {x.key}
            </button>
          ))}
      ...
      <div
        className="mx-4 mt-5 rounded-[6px] px-4 py-4"
        role="tabpanel"
        style={{
          boxShadow:
            "inset 2px 3px 8px rgba(0,0,0,0.6), inset -1px -1px 1px rgba(255,255,255,0.05)",
          background: "#171a1f",
        }}
      >
        <p className="font-body text-micro text-steel-300">{f.key}</p>
        <p className="struck-ink mt-1 font-display text-title font-extrabold leading-none tabular-nums">
          {f.value}
        </p>
      ...
      <ul className="mt-4 flex flex-col gap-1.5 px-4 font-body text-micro text-steel-300">
        {c.fields.map((x, k) => (
          <li key={x.key} className="flex items-center justify-between gap-3">
            <span>{x.key}</span>
            {k === i ? (
              <span className="text-steel-50">disclosed</span>
            ) : (
              <span aria-hidden="true" className="hatch w-14" />
            )}
          </li>
        ))}
      </ul>
