...
const RAIL = [
  "Reg. (EU) 2023/1542 · Art. 49(2)",
  "Passport mandatory 18 Feb 2027",
  "Art. 52 · due regard for business confidentiality",
];
  ...
  return (
    <section className="bg-ground text-ink">
      <HeroScene className="flex min-h-[100svh] flex-col">
        ...
        <div className="relative z-10 flex flex-1 flex-col justify-center px-[clamp(24px,4vw,64px)] pb-[14vh] pt-6 max-md:justify-start max-md:px-[22px] max-md:pb-6 max-md:pt-8">
          <div className="max-w-[640px]">
            <p className="reveal flex items-center gap-3 font-body text-[12px] font-medium uppercase tracking-[0.10em] text-ink-muted">
              <span aria-hidden="true" className="inline-block h-px w-7 bg-ink-muted" />
              Confidential product passport
            </p>
            <h1 className="reveal reveal-delay-1 mt-4 font-display text-[clamp(38px,10vw,54px)] font-extrabold leading-[0.98] tracking-[-0.015em] text-ink md:text-[clamp(40px,4vw,58px)]">
              Prove the threshold.
              <br />
              <em className="font-bold">Keep the recipe.</em>
            </h1>
            <p className="reveal reveal-delay-2 mt-5 max-w-[480px] font-body text-[16px] leading-normal text-ink md:text-[17px]">
        ...
        <footer className="relative z-10 flex min-h-14 flex-wrap items-center justify-between gap-x-6 gap-y-2 px-[clamp(24px,4vw,64px)] py-2 font-mono text-[11.5px] uppercase tracking-[0.08em] text-ink-muted tabular-nums max-md:px-[22px]">
          ...
          <span className="shrink-0 whitespace-nowrap max-lg:hidden">Open-source reference implementation</span>
        </footer>
      </HeroScene>
    </section>
  );
