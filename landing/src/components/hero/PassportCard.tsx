...
export function PassportCard({
  ...
  return (
    ...
    >
      <div
        className="passport-row flex items-center justify-between gap-4"
        style={{ transitionDelay: on ? "120ms" : "0ms" }}
      >
        <span className="flex items-center gap-2">
          <span aria-hidden="true" className="passport-seal" />
          <span className="engraved font-display text-small font-bold leading-none">
            {piece.name}
          </span>
        </span>
        <span className="whitespace-nowrap font-body text-micro text-ink-muted">
          {piece.regulation}
        </span>
      </div>

      <div
        className="passport-row mt-2 flex items-baseline justify-between gap-3"
      ...
      >
        ...
        <span className="flex items-center gap-1.5 whitespace-nowrap">
          ...
          <span className="font-display text-caption font-bold leading-none text-ok">
            Compliant
          </span>
        </span>
      </div>
      ...
      <ul
        className="passport-row mt-2 flex items-center gap-3"
      ...
      >
        {piece.sealed.map((row) => (
          <li key={row.label} className="flex min-w-0 items-center gap-1.5">
            ...
            <span
              aria-hidden="true"
              className="passport-hatch"
              style={{ width: `${Math.round(row.width * 0.5)}px` }}
            />
