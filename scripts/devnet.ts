...
export function summarizeStatus({ services, containers, probes, genesis, expectedGenesis }: StatusInput): string[] {
  const errors: string[] = [];
  ...
  for (const service of services) {
    ...
    else if (container.state !== 'running') errors.push(`${service}: container is ${container.state}`);
  ...
  else if (genesis !== expectedGenesis) errors.push(`node: genesis is ${genesis}, expected ${expectedGenesis}`);
