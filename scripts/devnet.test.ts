...
async function serve(handler: (body: string) => { status: number; body: string } | null): Promise<string> {
...
describe('parseComposePs', () => {
  ...
  it('reads a JSON array', () => {
    ...
    expect(parseComposePs(output)).toEqual([{ service: 'node', state: 'exited', health: undefined }]);
  });
...
describe('summarizeStatus', () => {
  ...
  it('rejects an endpoint that is not ready', () => {
    const input = validInput();
    input.probes[0] = { service: 'indexer', url: 'http://127.0.0.1:8088/ready', status: 503, ms: 4 };
