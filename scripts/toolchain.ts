  ...
  return readdirSync(join(root, directory), { withFileTypes: true })
    .flatMap((entry) => {
      ...
      if (entry.isDirectory())
        return SKIPPED_DIRECTORIES.has(entry.name) ? [] : compactFiles(root, path);
...
export function checkToolchain({ root, compiler, nodeVersion }: ToolchainInput): string[] {
  ...
  if (compiler.version !== pinned) {
    errors.push(
      `.compact-version is ${pinned}, but compact compile --version reports ${compiler.version}`,
    );
  }
  ...
  const manifests = [
    "package.json",
    ...(rootManifest.workspaces ?? []).map((w) => `${w}/package.json`),
  ];
  for (const file of manifests) {
    ...
    for (const field of ["dependencies", "devDependencies"] as const) {
      for (const [name, spec] of Object.entries(manifest[field] ?? {})) {
        ...
        if (!EXACT.test(spec))
          errors.push(`${file}: ${field}.${name} is "${spec}", expected an exact version`);
        else if (expected && spec !== expected)
          errors.push(`${file}: ${field}.${name} is ${spec}, expected ${expected}`);
      }
    }
  ...
  for (const name of SINGLE_COPY) {
    ...
    if (versions.length > 1) {
      errors.push(
        `package-lock.json: ${name} resolves to ${versions.join(", ")}, expected one version`,
      );
    }
  }
  for (const version of [...(resolved.get("@types/node") ?? [])].sort()) {
    if (!version.startsWith("24."))
      errors.push(`package-lock.json: @types/node resolves to ${version}, expected major 24`);
  }
  ...
  for (const file of compactFiles(root)) {
    ...
    if (pragma !== language) {
      errors.push(
        `${file}: pragma is ${pragma === undefined ? "missing" : `"${pragma}"`}, expected "${language}"`,
      );
    }
  }
  ...
  for (const [image, expected] of Object.entries(matrix.images)) {
    ...
    if (tag === undefined) errors.push(`${matrix.compose}: ${image} is missing`);
      ...
      errors.push(`${matrix.compose}: ${image} is ${tag}, expected ${expected}`);
  }
