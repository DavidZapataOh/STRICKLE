...
describe("checkToolchain", () => {
  ...
  it("rejects a compile script without the pinned compiler", () => {
    const root = repo((files) => {
      json(files, "contract/package.json").scripts.compact =
        "compact compile src/c.compact src/managed/c";
    });
    expect(checkToolchain({ root, compiler, nodeVersion })).toEqual([
  ...
  it("rejects a Midnight package outside the matrix", () => {
    const root = repo((files) => {
      json(files, "cli/package.json").dependencies["@midnight-ntwrk/midnight-js-contracts"] =
        "4.0.0";
    });
    expect(checkToolchain({ root, compiler, nodeVersion })).toEqual([
  ...
  it("rejects a compact-runtime that differs from the compiler runtime", () => {
    const root = repo((files) => {
      json(files, "contract/package.json").dependencies["@midnight-ntwrk/compact-runtime"] =
        "0.15.0";
    });
    expect(checkToolchain({ root, compiler, nodeVersion })).toEqual([
  ...
  it("rejects two resolved copies of a single-copy package", () => {
    const root = repo((files) => {
      json(files, "package-lock.json").packages["cli/node_modules/typescript"] = {
        version: "5.9.3",
      };
    });
    expect(checkToolchain({ root, compiler, nodeVersion })).toEqual([
  ...
  it("rejects Node types outside major 24", () => {
    const root = repo((files) => {
      json(files, "package-lock.json").packages["node_modules/@types/node"] = {
        version: "22.20.3",
      };
    });
    expect(checkToolchain({ root, compiler, nodeVersion })).toEqual([
  ...
  it("rejects a devnet image outside the matrix", () => {
    const root = repo((files) => {
      files["cli/docker-compose.yml"] = compose.replace(
        "midnight-node:1.0.2",
        "midnight-node:1.0.0",
      );
    });
